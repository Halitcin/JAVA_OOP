...
public class ParseFeed {
	...
	public static HashMap<String, Integer> CoronaDeathsFromCSV(PApplet p, String fileName) {
		// HashMap key: country ID and  data: corona death data
		HashMap<String, Integer> covidMap = new HashMap<String, Integer>();
		...
		for (String row : rows) {
			...
			//  get most recent death data from csv file
			
				covidMap.put(columns[0], Integer.parseInt(columns[columns.length - 1]));
					
		}
	
		for (String name : covidMap.keySet()) {
			System.out.println(name + " "+ covidMap.get(name));
